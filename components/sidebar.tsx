...
import {
  ...
  PiggyBank, Target, Shield, Settings,
  Brain, CreditCard, Building2, ChevronLeft, ChevronRight,
} from "lucide-react"
...
const navItems: NavItem[] = [
  ...
  { href: "/protect/cartoes",       label: "Cartões",       icon: CreditCard },
  { href: "/protect/bancos",        label: "Bancos",         icon: Building2 },
    ...
    <aside className={cn(
      ...
      <div className="flex items-center justify-between px-3 h-16 border-b border-sidebar-border">
        <Link
          ...
          className={cn("flex items-center gap-2 transition-all", mounted && collapsed && "justify-center w-full")}
        >
          <div className="flex items-center justify-center size-8 rounded-lg bg-primary shrink-0">
            <Brain className="size-4 text-primary-foreground" />
          </div>
          {(!mounted || !collapsed) && (
            <span className="font-semibold text-sidebar-foreground text-lg tracking-tight">MoneyMind</span>
          )}
        </Link>
        {(!mounted || !collapsed) && (
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              className={cn(
                ...
                active ? "bg-sidebar-primary/10 text-sidebar-primary" : "text-muted-foreground hover:text-sidebar-foreground hover:bg-sidebar-accent",
      ...
      <div className="border-t border-sidebar-border p-3">
        {(!mounted || !collapsed) ? (
          <div className="rounded-xl bg-sidebar-accent/50 p-4">
            ...
            <Button size="sm" className="w-full text-xs">Upgrade Pro</Button>
          </div>
        ) : (
          <Button variant="ghost" size="icon-sm" onClick={toggleSidebar}
